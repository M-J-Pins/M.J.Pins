...
async def revoke_access_to_wallet(
    request: AccessToWalletRequest = Depends(),
    db: AsyncSession = Depends(get_session),
):
    """
    Ручка принимает id пользователя, у которого надо забрать доступ к кошельку
    и id кошелька
    Забирает доступ, если запрос отправил владелец кошелька
    """
