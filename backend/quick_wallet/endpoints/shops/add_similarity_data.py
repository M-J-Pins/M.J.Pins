...
from quick_wallet.database.models import Shop, ShopCardColor, ShopCardText
...
async def add_similarity_data(
    ...
    Примерный текст на карте и/или основной цвет карты
    ...
    if request.card_text != "":
        await ShopCardText.create(db, shop_id=request.shop_id, text=request.card_text)
    if request.card_color != "":
        await ShopCardColor.create(
            db, shop_id=request.shop_id, color=request.card_color
        )
