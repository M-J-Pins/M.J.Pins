...
from quick_wallet.schemas.wallets import (
    WalletCardScheme,
    WalletListResponse,
    WalletResponse,
    WalletShortInfo,
...
class ConvertManager:
    ...
    @staticmethod
    async def wallet_id2wallet_scheme(
        db: AsyncSession, user_id: UUID, wallet_id: UUID
    ) -> WalletResponse:
        db_wallet: Wallet = await Wallet.get(db, id=wallet_id)

        user_ids: List[UUID] = [
            user_id for user_id in await WalletAccess.get_all(db, wallet_id=wallet_id)
        ]
        user_phones: List[str] = [
            f"+{(await User.get(db, id=user_id)).phone}" for user_id in user_ids
        ]

        db_card_ids: List[UUID] = [
            card.card_id for card in await WalletCard.get_all(db, wallet_id=wallet_id)
        ]
    ...
    @staticmethod
    async def user_id2wallet_list_scheme(
        db: AsyncSession, user_id: UUID
    ) -> WalletListResponse:
        wallet_ids: List[UUID] = [
            wallet_access.id
            for wallet_access in await WalletAccess.get_all(db, user_id=user_id)
        ]
